...
- 14.1 Design `workspaces` table (id, name, slug, owner, plan, created_at) as the billable tenant unit
- 14.2 Add `workspace_id` foreign key to `urls`, `clicks`, `tags`, `api_keys`, and `domains`
- 14.3 Write migrations (up/down) for workspaces and the new foreign keys; backfill existing rows into a default workspace
- 14.4 Auto-create a personal workspace on user signup so no account is ever workspace-less
- 14.5 Add "active workspace" resolution to the gRPC interceptor (from session, API key, or header)
- 14.6 Scope every repository query by `workspace_id` (no cross-workspace reads or writes)
- 14.7 Enable Postgres Row-Level Security as a defense-in-depth backstop to application-level scoping
- 14.8 Add a workspace switcher to the dashboard and persist the active workspace per session
...
- 15.1 Design `memberships` table (user_id, workspace_id, role, status, created_at)
- 15.2 Move role from a global user property to a per-membership property
- 15.3 Define workspace roles: Owner, Admin, Member, Viewer (read-only)
- 15.4 Enforce the "at least one Owner" invariant (block removing/demoting the last Owner)
- 15.5 Add an ownership-transfer flow
- 15.6 Design `invitations` table (workspace_id, email, role, token, invited_by, expires_at, status)
- 15.7 Build invite flow: create pending invite → email accept link → create membership on accept
- 15.8 Handle both cases: invitee already has an account vs. needs to sign up first
- 15.9 Add resend, revoke, and expiry-sweep for pending invitations
- 15.10 Member-management UI: list members, change roles, remove members, view pending invites

### [ ] Phase 16: Billing & Entitlements (Stripe)

[#--phase-16-billing--entitlements-stripe](#--phase-16-billing--entitlements-stripe)

- 16.1 Integrate Stripe; design `subscriptions` table keyed to `workspace_id`
- 16.2 Map Stripe price IDs to internal plan tiers (Free, Pro, Business, Enterprise)
- 16.3 Build an entitlements layer that resolves a plan into concrete limits (max links, max domains, retention days, feature flags)
- 16.4 Enforce quota checks in the gRPC interceptor alongside existing rate limiting
- 16.5 Define seat billing: each accepted non-Viewer membership = one billable seat; Viewers free
- 16.6 Sync Stripe subscription quantity when members are added or removed
- 16.7 Handle Stripe webhooks to keep subscription state in sync (created, updated, canceled, payment failed)
- 16.8 Usage metering for links created and clicks (for quota enforcement and future usage-based pricing)
- 16.9 Billing/subscription management page (current plan, usage vs. limits, upgrade/downgrade, invoices)
- 16.10 Quota-exceeded UX (clear messaging plus upgrade prompt rather than silent failure)

### [ ] Phase 17: Custom Domain Wizard (BYOD & Automatic TLS)

[#--phase-17-custom-domain-wizard-byod--automatic-tls](#--phase-17-custom-domain-wizard-byod--automatic-tls)

- 17.1 Design `domains` table (workspace_id, domain, status, verification_token, created_at)
- 17.2 Define the domain status state machine: pending_dns → verifying → provisioning_cert → active (plus a failed branch)
- 17.3 Support platform subdomains (`{workspace}.goshrtn.io`) via the existing wildcard cert — zero per-domain TLS work
- 17.4 Build the add-domain wizard: input domain → generate token → show CNAME + TXT records with copy buttons
- 17.5 Apex-domain detection at input (steer to a subdomain or switch instructions to an A/ALIAS record)
- 17.6 Background DNS polling and a status endpoint so the wizard self-verifies without repeated manual clicks
- 17.7 Ownership verification via TXT challenge and/or CNAME resolution check before activation
- 17.8 Live failure diagnostics (CNAME not found, points elsewhere, Cloudflare proxy detected, propagation pending)
- 17.9 Detect provider (e.g. Cloudflare) and surface the "DNS-only / grey-cloud" warning inline
- 17.10 Embed CertMagic for on-demand Let's Encrypt issuance and automatic renewal
- 17.11 Gate cert issuance on verified-domain status; rate-limit issuance to respect Let's Encrypt limits
- 17.12 Persist wizard progress so users can close and resume; show status badges in the domains list
- 17.13 Restrict the link-creation Domain dropdown to the workspace's verified/active domains only
- 17.14 Gate custom domains to paid plans via entitlements

### [ ] Phase 18: Signup Integrity & Account Lifecycle

[#--phase-18-signup-integrity--account-lifecycle](#--phase-18-signup-integrity--account-lifecycle)

- 18.1 Require email verification before a new account can create links (pending → verified states)
- 18.2 Add Cloudflare Turnstile (or equivalent) to the signup form
- 18.3 Rate-limit signups per IP using the existing Redis token buckets
- 18.4 Block known disposable/throwaway email domains
- 18.5 (Optional, hosted free tier) offer OIDC/social-only signup to raise the bar on throwaway accounts
- 18.6 Define the account lifecycle policy (unverified purge window, inactivity nudges, soft-disable, deletion)
- 18.7 Scheduled Go worker to enforce lifecycle (purge unverified, nudge dormant, anonymize/delete with notice)
- 18.8 URL reputation scanning on link creation (Google Safe Browsing API, URLhaus) to protect domain reputation
- 18.9 Throttle link creation for new/free accounts; auto-disable accounts that trip abuse heuristics
- 18.10 Activation onboarding: get a new user to their first working short link in under a minute, plus nudge emails

### [ ] Phase 19: Open-Core Licensing & Edition Split

[#--phase-19-open-core-licensing--edition-split](#--phase-19-open-core-licensing--edition-split)

- 19.1 Decide licensing strategy (keep Apache-2.0 core vs. relicense core to AGPL-3.0; source-available for the paid layer)
- 19.2 Adopt a CLA or DCO before merging further contributions to enable future relicensing
- 19.3 Move SaaS-only code (billing, multi-workspace tenancy, SAML/SCIM, audit logs, white-label) into a separate `/ee` directory under a distinct license
- 19.4 Keep the self-hosted core uncapped — no quota limits in open-source code (caps live only on hosted infra)
- 19.5 Keep basic OIDC in the open core; reserve SAML/SCIM/audit logs for the paid tier (avoid the "SSO tax" on basics)
- 19.6 Document the edition split (OSS self-hosted vs. EE/hosted) in the README
- 19.7 Build-time flag separation so the OSS build excludes EE features cleanly

### [ ] Phase 20: Premium Feature Gating & Plan Packaging

[#--phase-20-premium-feature-gating--plan-packaging](#--phase-20-premium-feature-gating--plan-packaging)

- 20.1 Finalize the tier structure and pricing (Free / Pro / Business / Enterprise, plus a self-hosted support offering)
- 20.2 Tie Phase 11 advanced link features (password protection, geo/device/time redirects, A/B testing) to plan entitlements on hosted
- 20.3 Tie Phase 13 theming, white-label, and branding-removal to paid tiers
- 20.4 Configure per-plan limits (links, domains, retention, rate limits, seats) in the entitlements layer
- 20.5 Add a self-hosted support / enterprise license offering (SSO, audit logs, SLA, paid support contracts)
- 20.6 Pricing/plans marketing page plus in-app upgrade prompts at quota and feature boundaries
